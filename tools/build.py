...
SITE = {
    ...
    "YEAR": "2026",
    "ORIGIN": "https://undistributedfund.com",
}

# Stamped into every submission. The intake Worker refuses a submission whose
# form_version it doesn't know, so bump this only together with a redeploy.
FORM_VERSION = "surplus-2026-09"
...
# Consent manifest.
#
# The intake Worker has to know what the checkbox said, character for
# character, to tell a real consent record from one describing language that
# was never on the page. Deriving it here means it cannot drift: the same
# string that renders in the form produces the manifest, every build.
#
# labelText() in site.js sends the label's textContent with whitespace
# collapsed. rendered_label() reproduces exactly that, server-side.
# ----------------------------------------------------------------------------
def rendered_label(badge, fragment):
    text = badge + " " + re.sub(r"<[^>]+>", "", fragment)
    return re.sub(r"\s+", " ", html.unescape(text)).strip()


def write_consent_manifest():
    terms = rendered_label("Required", TERMS_CONSENT)
    sms = rendered_label("Optional", SMS_CONSENT)
    sha = lambda s: hashlib.sha256(s.encode("utf-8")).hexdigest()
    manifest = {
        "_": "Generated by tools/build.py. Do not edit. Redeploy the Worker after a rebuild.",
        "form_version": FORM_VERSION,
        "terms": {"text": terms, "sha256": sha(terms)},
        "sms": {"text": sms, "sha256": sha(sms)},
    }
    out = ROOT / "worker" / "consent-manifest.json"
...
def form(p, h="h3"):
  ...
  <form class="intake" id="{p}-form" data-prefix="{p}" novalidate>
    ...
    <!-- Honeypot. Off-screen and out of the tab order, so no visitor meets it;
         a submission that fills it is recorded and flagged, never emailed.
         Named fax_number on purpose — no password manager autofills that. -->
    <div class="hp" aria-hidden="true">
      <label for="{p}-fax">Fax number</label>
      <input type="text" id="{p}-fax" name="fax_number" tabindex="-1" autocomplete="off">
    </div>
    <div class="consent">
    ...
    <!-- The consent record. site.js fills these; the endpoint rejects a
         submission whose consent flag is set but whose *_at timestamp is
         missing or out of order, because that flag was not set by a person. -->
    <input type="hidden" name="consent_terms_text"><input type="hidden" name="consent_sms_text">
    <input type="hidden" name="consent_terms_at"><input type="hidden" name="consent_sms_at">
    <input type="hidden" name="form_loaded_at"><input type="hidden" name="captured_at">
    <input type="hidden" name="page_url"><input type="hidden" name="referrer">
    <input type="hidden" name="user_agent"><input type="hidden" name="form_version" value="{FORM_VERSION}">
...
def render(src):
    ...
    # The homepage is canonical at the bare domain, not at /index.html.
    canonical = SITE["ORIGIN"] + "/" + ("" if out_name == "index.html" else out_name)
    out = f'''<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
if __name__ == "__main__":
    ...
    manifest = write_consent_manifest()
    print("consent manifest:", manifest.relative_to(ROOT))

    # The consent record is only worth having if the text in it is the text
    # that was on screen. Verify that against the built HTML on every build.
    import subprocess, sys
    print("checking consent text:", flush=True)
    rc = subprocess.run([sys.executable, str(ROOT / "tools" / "check_consent.py")]).returncode
    if rc:
        raise SystemExit("consent text check failed — do not deploy")
    print("\nIf the consent text changed, redeploy the Worker too:")
